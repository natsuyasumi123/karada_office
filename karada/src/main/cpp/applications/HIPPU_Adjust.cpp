...
void HIPPU_Adjust::Init()
{
	...
	{
		LOGCATE("HIPPU_Adjust::Init create program fail");
	}
...
void HIPPU_Adjust::LoadImage(NativeImage *pImage)
{
	LOGCATE("HIPPU_Adjust::LoadImage pImage = %p", pImage->ppPlane[0]);
	if (pImage)
	{
...
void HIPPU_Adjust::initHippuData() {
    std::vector<float> kaoData(5 * 3 ) ;
    bool ret = MyGLRenderContext::GetInstance()->getKaradaData(0 , 4 ,kaoData);
    if(ret){
        ...
        LOGCATE("MUNE_Burst::Init create program success");
    }
}

void HIPPU_Adjust::Draw(int screenW, int screenH)
{
	LOGCATE("HIPPU_Adjust::Draw() [w,h]=[%d,%d]", screenW, screenH);
    ...
    LOGCATE("HIPPU_Adjust::Draw() ratio=%f, effectRadius=%f", ratio, effectRadius);
 ...
 * @param ratio 宽高比
 * */
void HIPPU_Adjust::UpdateMVPMatrix(glm::mat4 &mvpMatrix, int angleX, int angleY, float ratio)
{
	LOGCATE("HIPPU_Adjust::UpdateMVPMatrix angleX = %d, angleY = %d, ratio = %f", angleX, angleY, ratio);
	angleX = angleX % 360;
	angleY = angleY % 360;
...
void HIPPU_Adjust::UpdateTransformMatrix(float rotateX, float rotateY, float scaleX, float scaleY)
{
