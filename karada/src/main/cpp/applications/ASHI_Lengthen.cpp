...
void ASHI_Lengthen::LoadImage(NativeImage *pImage)
{
	LOGCATE("ASHI_Lengthen::LoadImage pImage = %p", pImage->ppPlane[0]);
	if (pImage)
	{
...
//	};
	...
	if(m_FboProgramObj) {
		...
		switch (m_StretchMode)
		{
			...
			case HORIZONTAL_STRETCH_RIGHT_6_POINTS:
				...
				break;
			default:
				LOGCATE("ASHI_Lengthen::Init don't support this mode");
				return;
		}
	...
	{
		LOGCATE("ASHI_Lengthen::Init m_ProgramObj == GL_NONE");
		return;
	}
	...
	{
		LOGCATE("ASHI_Lengthen::Init CreateFrameBufferObj fail");
		return;
	}
}
...
bool ASHI_Lengthen::CreateFrameBufferObj()
{
	glGenTextures(1, &m_FboTextureId);
	glBindTexture(GL_TEXTURE_2D, m_FboTextureId);
	glTexParameterf(GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, GL_CLAMP_TO_EDGE);
	...
	if (glCheckFramebufferStatus(GL_FRAMEBUFFER)!= GL_FRAMEBUFFER_COMPLETE) {
		LOGCATE("ASHI_Lengthen::CreateFrameBufferObj glCheckFramebufferStatus status != GL_FRAMEBUFFER_COMPLETE");
		return false;
	}
	glBindTexture(GL_TEXTURE_2D, GL_NONE);
	glBindFramebuffer(GL_FRAMEBUFFER, GL_NONE);
