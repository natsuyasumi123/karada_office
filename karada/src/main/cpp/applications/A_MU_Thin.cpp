...
void A_MU_Thin::Init()
{
	...
	{
		LOGCATE("A_MU_Thin::Init create program fail");
	}
...
void A_MU_Thin::LoadImage(NativeImage *pImage)
{
	LOGCATE("A_MU_Thin::LoadImage pImage = %p", pImage->ppPlane[0]);
	if (pImage)
	{
...
void A_MU_Thin::Draw(int screenW, int screenH)
{
	LOGCATE("A_MU_Thin::Draw() [w,h]=[%d,%d]", screenW, screenH);
    ...
    LOGCATE("A_MU_Thin::Draw() ratio=%f, effectRadius=%f", ratio, effectRadius);
 ...
 * @param ratio 宽高比
 * */
void A_MU_Thin::UpdateMVPMatrix(glm::mat4 &mvpMatrix, int angleX, int angleY, float ratio)
{
	LOGCATE("A_MU_Thin::UpdateMVPMatrix angleX = %d, angleY = %d, ratio = %f", angleX, angleY, ratio);
	angleX = angleX % 360;
	angleY = angleY % 360;
...
void A_MU_Thin::UpdateTransformMatrix(float rotateX, float rotateY, float scaleX, float scaleY)
{
