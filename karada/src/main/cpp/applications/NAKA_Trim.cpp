...
void NAKA_Trim::initNakaData(){
    std::vector<float> kaoData(5 * 3 ) ;
    bool ret = MyGLRenderContext::GetInstance()->getKaradaData(0 , 5 ,kaoData);
    if(ret){
...
void NAKA_Trim::Init()
{
	...
	{
		LOGCATE("NAKA_Trim::Init create program fail");
	}
...
void NAKA_Trim::LoadImage(NativeImage *pImage)
{
	LOGCATE("NAKA_Trim::LoadImage pImage = %p", pImage->ppPlane[0]);
	if (pImage)
	{
...
void NAKA_Trim::Draw(int screenW, int screenH)
{
	LOGCATE("NAKA_Trim::Draw() [w,h]=[%d,%d]", screenW, screenH);
 ...
 * @param ratio 宽高比
 * */
void NAKA_Trim::UpdateMVPMatrix(glm::mat4 &mvpMatrix, int angleX, int angleY, float ratio)
{
	LOGCATE("NAKA_Trim::UpdateMVPMatrix angleX = %d, angleY = %d, ratio = %f", angleX, angleY, ratio);
	angleX = angleX % 360;
	angleY = angleY % 360;
...
void NAKA_Trim::UpdateTransformMatrix(float rotateX, float rotateY, float scaleX, float scaleY)
{
